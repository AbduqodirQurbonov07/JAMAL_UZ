        ...
        <Popover>
          ...
          <PopoverContent class="cursor-pointer flex flex-col rounded-xl w-52">
            <Dialog class="w-[800px]">
              ...
              <DialogContent class="max-w-[700px]">
                <DialogHeader class="border-b pb-4">
                  <DialogTitle>Omborxona tahrirlash</DialogTitle>
                </DialogHeader>
                <ul class="grid grid-cols-1 gap-3">
                  <li class="flex flex-col gap-1.5 mt-2.5 mb-5">
                    <Label for="size"
...
const newWarehouseCom = ref("");
const editWarehouse = async (e: any) => {
  const token = localStorage.getItem("token");
  console.log(e.target.id);
  const payload = {
